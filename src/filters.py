"""Filtering: keep only target US roles that match config-driven title rules.

A job passes only if it is (1) compatible with the configured role rules,
...
def passes(job: Job, f: dict) -> bool:
    ...
    global_excludes = [t.lower() for t in role_cfg.get("global_exclude_terms", [])]
