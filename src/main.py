...
log = logging.getLogger("fulltime_pos_emailer")
...
def _print_digest(jobs: list[Job]) -> None:
    ...
    print("\n" + "=" * 70)
    print(f"  {len(jobs)} NEW matching full-time job(s)")
    print("=" * 70)
...
def main(argv: list[str] | None = None) -> int:
    _setup_logging()
    parser = argparse.ArgumentParser(description="Daily full-time job scraper + notifier")
