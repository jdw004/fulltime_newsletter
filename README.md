...
3. (Optional) Run `python -m src.main --seed` locally once and commit the updated
   ...
   the whole backlog.
4. The workflow `.github/workflows/daily.yml` runs every 1.5 hours at **9:05 AM,
   10:35 AM, 12:05 PM, 1:35 PM, 3:05 PM, 4:35 PM, and 6:05 PM America/New_York daily** and also on-demand from the **Actions tab**
...
- Adjust the time by editing the `cron:` line and `timezone:` value.
...
It sends a short "N new full-time jobs today" text alongside the email digest. Twilio
